...
// 安装任务表：单体版在 lib/index.js（路由与测试共用同一模块实例），分层版在 lib/server/state.js
...
try {
  ...
  //    pnpm remove 包目录 + 回读核实（真 pnpmRemove 不打桩，钉死"路由 → 撤销逻辑"的参数接线）。
  {
    ...
    if (jobs !== null && reinstalled) {
      // 现场与安装成功后的状态一致：patch insert 行（appendInsert）+ bundles 清单（addBundleToManifest）
      const ROW = 'left-pad'
