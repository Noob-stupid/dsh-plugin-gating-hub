  ...
  for (const [sourceIndex, cand] of candidates.entries()) {
    ...
    for (let round = 0; round < 2; round += 1) {
      ...
      if (res.timedOut === true && round === 0 && hasProgress) {
        ...
        errors.push({ url, message: `克隆超时（${useTimeout}ms，本次已收到 ${bytesReceived} B），改用 ${retryTimeout}ms 同源重试`, stderr: res.stderr, timedOut: true, retrying: true, bytesReceived, dir: part, ...trashField })
        continue
      }
      errors.push({
        url,
        message: res.timedOut === true
          ...
          : (res.stderr !== '' ? res.stderr : `git clone 退出码 ${res.code}`),
        stderr: res.stderr,
        timedOut: res.timedOut === true,
        bytesReceived,
...
export { reposDirCache, getReposDir, setReposDir, listLandedRepos, gitCloneRepo, summarizeCloneErrors, orderGitCandidates, readGitSourceMemo, rememberGitSource, killProcessTree, probeSourceAlive, GIT_STALL_ARGS, GIT_CLONE_TIMEOUT_MS, measureDirBytes, measureProgressBytes }
