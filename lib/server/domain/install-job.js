...
async function tryCandidateChannels({ job, ch, name, profileDir, registries, repoChannelAllowed, budget, baseUrl = null, expanded = false, deadline = null, expand = null }) {
  ...
  // 2026-09-27（改错，批次 A-④）：整段（含其后紧跟的 backfillMissingDeps）必须包 try/catch ——
  // 旧代码在这里不设防：竞速实现抛一次异常就直接把作业判 failed，**后面的串行通道一个都不再试**。
  {
    try {
  ...
  for (let ri = 0; ri < registries.length && installedName === null; ri += 1) {
    try {
      ...
      installedName = name
      break
    } catch (error) { lastError = error; pnpmError = error }
  }
  if (installedName === null) {
    ...
    try {
      ...
      job.integrity = info.integrity ?? null // ④ 下载物摘要校验结果（加法；见 install-verify.js）
    } catch (curlError) {
      lastError = curlError
    }
  }
  if (installedName === null) {
    ...
    if (budget.release > 0) {
      ...
      // 预算用尽（批次 A-⑤，2026-09-27 改错）：原因**总是**记在 job 上，供面板如实显示。
      // 旧代码只在 lastError === null 时才写一句 —— 于是最常见的情形（curl/pnpm 也失败了）
      // 面板上完全看不到"release 通道其实是因为预算被跳过"，用户只能猜。
      noteChannel(job, `release 通道因预算跳过（本作业只对前 ${RELEASE_CHANNEL_BUDGET} 个候选做按包名反查+release 扫描）`)
      // 但 lastError 仍保留旧语义：不覆盖真实错误（面板/AI 兜底要看的是 curl·pnpm 的失败原因），
      // 只在没有更具体错误时才把它当作最终错误。
      if (lastError === null) lastError = new Error(`release 通道候选预算已用尽（本作业只对前 ${RELEASE_CHANNEL_BUDGET} 个候选做按包名反查+release 扫描）`)
    }
  }
  // 懒惰展开（2026-09-27 搬家，批次 A-③）：**registry 类通道全部失败之后、git 通道之前**。
  // 为什么改顺序（真机事故：装 zhu1090093659/dsh-web 全家桶）：
  //   旧顺序是"所有通道（含 git）都失败 → 才展开子包"。而该仓库根包 private、未发布到 npm，
  //   于是 registry 通道必然 404 → 直接掉进 git 通道去 clone 那个 **429 MB** 的巨仓
  //   （ghproxy 上 git 协议 0 B/s，白等几分钟而且注定失败），真正能装的子包
  //   `@linxin666/dsh-web-all`（5.97 MB）**根本没机会被尝试**。
  //   搬到 git 之前后：registry 404 → 立刻展开子包 → 命中聚合子包 → 直接装完，全程不碰巨仓。
  // `expand()` 由调用方注入（它负责读 trees/package.json、去重、更新 candidateTotal），
  // 返回**新增候选数**；展开自身失败只记备注，绝不让整次安装崩掉（与通道 0 的 try/catch 同精神）。
  ...
  if (installedName === null && pnpmError !== null) {
    ...
    installedName = retry.installedName ?? installedName
    if (retry.error != null) lastError = retry.error
  }
...
async function runInstallJob(job, ports, deps = {}) {
  // 测试注入缝（2026-09-27 加法，沿用 channelImpls 的既有风格）：市场探测（仓库 package.json /
  // 子包列表）与两个时间预算可替换，**只为离线断言**「候选顺序、展开时机、预算边界」；
  // 生产调用方（routes/install.js 等）不传第三个参数，默认值与旧行为完全一致。
  // 为什么需要它：本机 github.com 与 api.github.com 都不可达（hosts 被加速器改过），
  // 而「private 根包 → 展开子包 → 按包名直装」这条链路的判据全在探测结果里，不注入就没法稳定验证。
  const probes = {
    fetchRepoPackage,
    fetchRepoPackageEx,
  ...
  try {
    ...
    for (let index = 0; index < candidates.length && installedName === null; index += 1) {
      ...
      // 批次 A-③（2026-09-27）：再叠加 gitChannelBlocked —— 根包未发布到 npm（或首选候选来自
      // 市场索引）时不试 git：git 通道只 clone 整个仓库，真机 dsh-web = 429 MB / ghproxy 0 B/s，纯白等。
      const repoChannelAllowed = (!subpackageMode || job.packageName === null || name === job.packageName) && job.gitChannelBlocked !== true
      // 懒惰展开的注入实现（搬到 registry 通道之后、git 通道之前，理由见 tryCandidateChannels 里那段注释）。
      // 只展开一次（expanded 由这里翻转，后续轮次传进去的就是 true）。
      const expandSubpackages = async () => {
        if (expanded || !job.repo) return 0
        expanded = true
        let subs = await probes.fetchSubpackageNames(job.repo, 'main', readGithubAuth().token)
        if (subs.length === 0) subs = await probes.fetchSubpackageNames(job.repo, 'master', readGithubAuth().token)
      ...
      const attempt = await tryCandidateChannels({
        ...
        expand: expanded ? null : expandSubpackages,
      })
    ...
    // ④ 装后校验（2026-09-26 加法，见 install-verify.js）：只**提示**、绝不改成功判定 ——
    // 入口文件缺失 / 补丁行指向别的包 / bundle 引用缺失 / 镜像未同步，都会写进 job.entryCheck 供面板显示
    job.entryCheck = verifyInstalledEntry(profileDir, installedName)
...
async function pnpmRemove(profileDir, packageName) {
  ...
  const opts = {
    ...
    // 2026-09-26：env 统一走 buildPnpmEnv（在既有 COREPACK_NPM_REGISTRY / 禁交互之上追加
    // npm_config_fetch_timeout / npm_config_fetch_retries / CI=true）。**不加** --fetch-timeout 类
    // CLI 选项：实测 `pnpm remove --fetch-timeout=…` 会 `[ERROR] Unknown options` 直接失败。
    env: buildPnpmEnv('https://registry.npmmirror.com'),
  }
