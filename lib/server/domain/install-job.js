 ...
 *    级联顺序也不该被破坏）。 */
async function tryCandidateChannels({ job, ch, name, profileDir, registries, repoChannelAllowed, budget, baseUrl = null, expanded = false, deadline = null }) {
...
async function runInstallJob(job, ports, deps = {}) {
  ...
  try {
    ...
    for (let index = 0; index < candidates.length && installedName === null; index += 1) {
      ...
      const attempt = await tryCandidateChannels({ job, ch, name, profileDir, registries, repoChannelAllowed, budget, baseUrl: ports.baseUrl ?? null, expanded, deadline })
