...
async function tryCandidateChannels({ job, ch, name, profileDir, registries, repoChannelAllowed, budget, baseUrl = null, expanded = false, deadline = null, expand = null }) {
  ...
  // 2026-09-27（批次 A-④）：整段（含其后的 backfillMissingDeps）必须包 try/catch —— 旧代码里竞速抛一次异常就把作业判 failed，后面的串行通道一个都不再试（本来可能装得上）。
  {
    try {
  ...
  if (installedName === null) {
    ...
    if (budget.release > 0) {
      ...
      // 预算用尽（批次 A-⑤）：原因**总是**记在 job 上供面板显示（旧代码只在 lastError === null 时写，
      // 而最常见的情形是 curl/pnpm 也失败 —— 用户根本看不出 release 通道没试）。
      noteChannel(job, `release 通道因预算跳过（本作业只对前 ${RELEASE_CHANNEL_BUDGET} 个候选做按包名反查+release 扫描）`)
      // lastError 保留旧语义：不覆盖真实错误，只在没有更具体错误时才用它
...
async function runInstallJob(job, ports, deps = {}) {
  // 测试注入缝（2026-09-27 加法，沿用 channelImpls 风格）：市场探测与两个时间预算可替换，只为离线断言
  // 「候选顺序、展开时机、预算边界」；生产调用方不传第三个参数，默认值与旧行为完全一致。
  // 需要它是因为本机 github.com / api.github.com 都不可达（hosts 被加速器改过），不注入没法稳定验证。
  const probes = {
    fetchRepoPackage,
    fetchRepoPackageEx,
  ...
  try {
    ...
    for (let index = 0; index < candidates.length && installedName === null; index += 1) {
      ...
      const expandSubpackages = async () => {
        ...
        const subs = await probes.fetchSubpackageNames(job.repo, job.defaultBranch ?? 'main', readGithubAuth().token)
