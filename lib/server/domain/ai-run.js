  ...
  switch (step.action) {
    ...
    case 'install-npm': {
      ...
      const { runner } = await runPnpmWithFallback(args, {
        // 2026-09-26：与安装主链路同一套 pnpm env（fetch 超时/重试 + CI=true 非交互），见 infra/exec.js
        execOpts: { timeout: 900000, windowsHide: true, maxBuffer: 4 * 1024 * 1024, env: buildPnpmEnv('https://registry.npmmirror.com'), signal },
      })
