...
(error) => setMessage(t("failed") + "：" + friendlyGithubError(error).message),
		...
		function PluginSafetyTab({ t }) {
			...
			react.useEffect(() => { load(); loadBackfill(); }, []);
			...
			const modeLabel = mode === "managed" ? t("safetyModeManagedShort") : t("safetyModeObserverShort");
			const pending = backfill !== null && typeof backfill.count === "number" ? backfill.count : 0;
			// 长按钮：默认只看到这一行（点开抽屉才展开全部内容）
			const trigger = el("button", { type: "button", "aria-expanded": open ? "true" : "false", onClick: () => setOpen((prev) => !prev), style: { width: "100%", maxWidth: "760px", display: "flex", alignItems: "center", gap: "14px", padding: "14px 16px", borderRadius: "14px", border: "1px solid var(--dsw-alias-border-secondary, rgba(127,127,127,.18))", background: "var(--dsw-alias-bg-elevated, rgba(127,127,127,.06))", cursor: "pointer", textAlign: "left", color: "inherit" } },
				el("div", { style: { width: "40px", height: "40px", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--dsw-alias-bg-secondary, rgba(127,127,127,.12))", fontSize: "18px", flex: "0 0 auto" } }, "🛡️"),
				el("div", { style: { flex: "1 1 auto", minWidth: "0" } },
					el("div", { style: { fontSize: "14px", fontWeight: 500 } }, t("safetyTab")),
					el("div", { style: { fontSize: "12.5px", opacity: .72, marginTop: "2px" } }, modeLabel + " · " + t("safetyFrameworkTitle") + " " + fw + (pending > 0 ? " · " + fill(t("safetyBackfillShort"), [["n", pending]]) : ""))),
				el("span", { style: { opacity: .55, flex: "0 0 auto" } }, open ? "⌄" : "›"));
			const drawerBody = el("div", { style: { position: "fixed", top: 0, right: 0, bottom: 0, width: "min(560px, 92vw)", overflowY: "auto", padding: "16px", background: "var(--dsw-alias-bg-layer-1, #1b1b1f)", boxShadow: "-8px 0 24px rgba(0,0,0,.35)", zIndex: 61 } },
				el("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" } },
					el("strong", { style: { fontSize: "15px" } }, t("safetyTab")),
					el("button", { type: "button", className: styles.toggle, onClick: () => setOpen(false) }, t("closeModal"))),
				el("div", { style: { display: "flex", flexDirection: "column", gap: "12px" } },
					...
					card("🧾", t("safetyBackfillTitle"), pending > 0 ? fill(t("safetyBackfillDesc"), [["n", pending]]) : t("safetyBackfillNone"), pending > 0 ? el("button", { type: "button", className: styles.toggle, disabled: busy, onClick: applyBackfill }, fill(t("safetyBackfillBtn"), [["n", pending]])) : null),
			...
			return el("div", null,
				trigger,
				open ? el("div", { style: { position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 60 }, onClick: () => setOpen(false) }, drawerBody) : null);
		}
		function apply(ctx) {
