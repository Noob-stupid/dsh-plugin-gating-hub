  ...
  try {
    ...
    if (installedName === null) {
      ...
      if (decision.approved === true) {
        ...
        const cleanedNote = leftovers.cleaned.length > 0 ? `；已清理本次落盘残留：${leftovers.cleaned.join('、')}` : ''
        const failedNote = leftovers.failed.length > 0
          ? `；**有 ${leftovers.failed.length} 项没能清理**（当前环境可能禁止删除，请手动删除）：${leftovers.failed.map((f) => f.path).join('、')}`
          : ''
        job.error = `${base}${cleanedNote}${failedNote}`
      }
      return
    }
